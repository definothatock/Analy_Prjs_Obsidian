
***List of near-orthogonal (conceptually) attributes, in different dimensions for better analysis and comparison.***

---

# "Genre" Attribute Categories
***These should really be Separated. But conventionally they are put in the same basket.***

### Player unit dynamics:
- Things about the dynamics between the real players (and computer).
  Defines By: Actor Relations
	
	#CoOp - has element of playing ***along*** other player
	#PvP - has element of playing ***against*** other player
	#PvE - has element of playing ***against*** Computer
	#Single - negation of the above


### Mechanics Subcategories:
- Player actions and goals; unit of Gameplay / Interactions.
  Defines By: Game Rules.
	
	Moment-to-Moment Dynamic (Core Mechanics):
	- UnGrouped
		#Carder - Throw cards
		#Platformer - Jumping between platforms
		#RTS - Strategies with no time stop
	
	- Shooters
		#FPShooter - 
		#TPShooter
	
	Progression/Loop Mode (Progression Mechanics):
	- UnGrouped
	#DeckBuilder - Build Card Decks
	
	- Exploration
		#OpenWorld - Progress with Exploring the world
		#RougeLike - Loops with heavy randomization
		#Metrovania - 2D exploration with progression locks



### Aesthetic:
- How the game "looks".
  Defines By: Rendering
	
	**Rendering-Art style:**

---

# Reward Mode
***limited to game internal convection. Exclude all external flows, e.g. topup.***

[[Simple Winning]] - Winning the round / level / game, no other effective rewards

### Positive reward
rewards that creates positive feedback(s), which are not limited to a loop/round/level.

### Social reward
rewards that does not affect the game, but have strong Social value









---
**Reference**: 

Game Mechanics & Systems Thinking
https://www.youtube.com/watch?v=nkLmjJK3vOw&list=PLyTIzr2AVP3_0zeFw9-9tjQNcV2aCnggd&index=3&t=408s

Game Genres - a Design Perspective
https://www.youtube.com/watch?v=3KJbYdNP5js&list=PLyTIzr2AVP3_0zeFw9-9tjQNcV2aCnggd&index=5&t=740s